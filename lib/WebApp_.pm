...
sub showstatic {
    my $self = shift;
    my $page_id = $self->query->param('page_id');
    return $self->tt_process( { 'page' => $self->_get_page( $page_id ) } );
}
...
#     $volume_id = $self ->_get_volume
   ...
   # my $pdf = $current->{'docpath'}.'/'.$article->{'paperid'}.'.pdf'; 
...
#     my $sql = 'SELECT * FROM public.papers WHERE journal = ? AND volume = ? AND issue = ? AND paperid IS NOT NULL ORDER BY start_page';
    ...
    my $sql = 'SELECT *, papers.journal as journal FROM public.papers LEFT OUTER JOIN virtual_themes ON papers.virtual_theme=virtual_themes.virtual_theme_id WHERE papers.journal = ? AND volume = ? AND issue = ? AND paperid IS NOT NULL ORDER BY start_page';
...
sub _get_theme_papers {
    ...
    my $sql = 'SELECT * FROM public.papers LEFT OUTER JOIN virtual_themes ON papers.virtual_theme=virtual_themes.virtual_theme_id WHERE papers.journal = ? AND virtual_theme = ? AND papers.paperid IS NOT NULL ORDER BY volume, start_page';
    # new version to accomodate 'virual themes'   (jb 4/11)
...
sub _get_article {
    ...
    my $sql='SELECT *, papers.journal as journal FROM public.papers LEFT OUTER JOIN virtual_themes ON papers.virtual_theme=virtual_themes.virtual_theme_id WHERE paperid = ?';
...
sub _get_authors {
    my ( $self, $paper_id ) = @_;
    my $sql = 'SELECT first_name, last_name, address FROM public.authors LEFT JOIN public.affil USING (affilnum,paperid) WHERE public.authors.paperid = ? ORDER BY position';
